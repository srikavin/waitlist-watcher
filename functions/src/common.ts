...
export const snapshotsIngestTopic = pubsub.topic("snapshots-ingest");
...
export const shardQueueUrl = (url: string, count: number) =>
    Array.from(url).map(x => x.charCodeAt(0)).reduce((a, b) => a + b, 0) % count
...
export const webhookQueue = (shard: number) => tasksClient.queuePath("waitlist-watcher", "us-east4", "webhook-queue-shard-" + shard)
